...
Filesystem::find_program(string_view program)
{
    ...
    if (!Filesystem::is_executable(program)) {
        auto searchpaths = searchpath_split(OIIO::Sysutil::getenv("PATH"));
        std::string path = Filesystem::searchpath_find(program, searchpaths);
#    ifdef _WIN32
        if (path.empty() && !Strutil::ends_with(program, ".exe"))
            path = Filesystem::searchpath_find(Strutil::concat(program, ".exe"),
                                               searchpaths);
#    endif
    }
    if (path.empty())
        return path;
    const auto permissions = filesystem::status(path).permissions();
    return path;
#endif
}
