...
JpgInput::read_native_scanline(int subimage, int miplevel, int y, int /*z*/,
                               void* data)
{
    return read_native_scanlines(
        subimage, miplevel, y, y + 1,
        make_span<std::byte>((std::byte*)data, m_spec.scanline_bytes(true)));
}
                                ...
                                int yend, int z, void* data)
{
    return read_native_scanlines(subimage, miplevel, ybegin, yend,
                                 span_from_buffer(data, m_spec.format,
                                                  m_spec.nchannels,
                                ...
                                int yend, span<std::byte> data)
{
    ...
    size_t sz       = sl_bytes * size_t(yend - ybegin);
    if (sz > data.size()) {
        errorfmt(
            "JPEG read_native_scanlines: Buffer size is inadequate ({} bytes provided vs {} needed)",
            data.size(), sz);
        return false;
    }
