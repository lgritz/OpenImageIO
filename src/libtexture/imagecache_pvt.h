...
#    endif
    ...
    void remember_tile(const TileID& id) {
        ...
        if (++m_thread_tile_count > PERTHREAD_CACHE) {
            ...
            ThreadTileMap newmap;
            for (ThreadTileMap::iterator i = m_thread_tiles.begin(),
                                         e = m_thread_tiles.end();
                 i != e; ++i) {
                if (i->second->used())
                    newmap.emplace(i->first, i->second);
                // FIXME: This is clearly wrong. Think hard about what the
                // policy is for deleting things from the local cache so that
                // we don't end up with tiles stranded here.
            }
            newmap.swap(m_thread_tiles);
        }
